...
function checkForm() {
	if (gridSizeEl.value < winCountEl.value) {
		...
		return alert("Le mode Hardcore est actuellement en maintenance :/ ");
	}
	else if (lvlAIEl.value != 1 && gridSizeEl.value > 3) {
		return alert("Le mode Hardcore n'est pas disponible en " + gridSizeEl.value + "x" + gridSizeEl.value);
	}
	else {
		startGame();
	}
